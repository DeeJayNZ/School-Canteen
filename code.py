...
@view("tandc")
def terms_page():
    pass # displays the website without canging anything in my function or returning anything.
...
def restock_success(food_id): # function gets food_id so it can be used inside the function
    food_id = int(food_id)
    found_food = None
    for food in canteen_test: # this crearted a for loop, for every item of food in canteen_test DO
        if food.id == food_id: #this ceates an if loop, basically is the two food_id s match then it will continue.
            found_food = food
...
@route("/table")
@view("table")
def table(): # this function is for my table, doesnt do much just has a dictionary so i can display the food on the webpage.
